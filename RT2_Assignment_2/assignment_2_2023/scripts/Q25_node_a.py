...
# Service node(b) callback
def last_target_callback():
    last_target = rospy.ServiceProxy('last_target', SetBool)
    try:
        response = last_target(True) 
        if response:
            return True
        else:     
            return False
...
def main():
    ...
    # Prompt the user for the averaging window size
    window_size = int(input("Enter the averaging window size: "))
    
    # Publisher for the averaging window size
    avg_window_size = rospy.Publisher('/window_size', Int32, queue_size=1)
    # Publish the averagin window size
    avg_window_size.publish(Int32(window_size))
    
    # Publisher for the goal coordinates x and y
    goal_position = rospy.Publisher('/goal_position', Point, queue_size=1)
    ...
    while not rospy.is_shutdown():
        ...
        current_goal_position.x = x
	current_goal_position.y = y
        goal_position.publish(current_goal_position)
	    ...
	    # Check the status of the goal		
            ...
            if goal_status == GoalStatus.ACTIVE:
                ...
                # Ask if the user wants to cancel the goal or get the last target coordinates
                user_input = input("Do you want to cancel the goal or receive the last target positions? (yes/no or l): ").lower()
                if user_input == "yes":
