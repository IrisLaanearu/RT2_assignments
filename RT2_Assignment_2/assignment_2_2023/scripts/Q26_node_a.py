...
# Service reset world callback
def reset_world_callback():
	reset_world = rospy.ServiceProxy('/gazebo/reset_world', Empty)
	try:
		response = reset_world() # No arguments
		if response:
			return True
		else:     
			return False
	...
	while not rospy.is_shutdown():
		...
		while not rospy.is_shutdown():
			...
			if goal_status == GoalStatus.ACTIVE:
				...
				# Ask if the user wants to cancel the goal or reset simulation
				user_input = input("Do you want to cancel the goal or reset simulation? (yes/no or r): ").lower()
				if user_input == "yes":
